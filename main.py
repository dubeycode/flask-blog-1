...
def login():
    ...
    return render_template("login.html", params=params)  # Show login page if GET request
...
def dashboard():
    ...
    return render_template("dashboard.html",params=params,posts=posts)  # Show dashboard if logged in
    ...
    if 'user' in session and session["user"] == params["admin_user"]:
        ...
        if request.method == 'POST':  # If form is submitted
            ...
            if sno == '0':  # Creating a new post
                new_post = Posts(title=box_title, slug=slug, content=content, subtitle=subtitle,
